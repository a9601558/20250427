...
- **isPaid**: BOOLEAN (是否付费)
- **price**: DECIMAL(10,2) (価格，如果isPaid=true)
...
**問題描述**: 前端与后端交互时的ID类型不一致导致新建题目无法保存到数据库。
...
**問題描述**: 相同的API路径在不同路由中被重复定义导致请求无法正确路由。
