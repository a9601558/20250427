...
export const safeEmit = (room: string, event: string, data: any): boolean => {
  try {
...
export const initializeSocket = (io: SocketIOServer) => {
  try {
    ...
    io.on('connection', (socket: Socket) => {
      ...
      // 监听用户访问权限检查事件
      socket.on('questionSet:checkAccess', async (data: { userId: string, questionSetId: string, forceRefresh?: boolean }) => {
        try {
          const user = await User.findByPk(data.userId);
          const questionSet = await QuestionSet.findByPk(data.questionSetId);
      ...
      // 处理用户购买记录同步请求
      socket.on('user:syncPurchases', async (data: { userId: string, deviceId?: string }) => {
        try {
