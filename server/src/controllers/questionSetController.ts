  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (questionData.options && Array.isArray(questionData.options)) {
        ...
        console.log(`问题ID: ${createdId}, 选项数据中的questionId:`, optionsData.map((opt: any) => opt.questionId));
        
        // 验证每个选项都有questionId
        const missingQuestionId = optionsData.filter((opt: any) => !opt.questionId);
      ...
      try {
        ...
        for (const questionSetId of questionSetIds) {
          try {
            ...
            const count = result && typeof (result as any).count !== 'undefined' 
              ? parseInt((result as any).count, 10) 
              : 0;
...
export const getQuestionSetById = async (req: Request, res: Response) => {
  try {
    ...
    if (responseData.questionSetQuestions && responseData.questionSetQuestions.length > 0) {
      ...
      responseData.questionSetQuestions.slice(0, 5).forEach((q: any, i: number) => {
...
export const updateQuestionSetQuestions = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const questionCount = await Question.count({ where: { questionSetId: req.params.id } });
      const responseData = updatedQuestionSet ? updatedQuestionSet.toJSON() : null;
      if (responseData) {
        (responseData as any).questionCount = questionCount;
      }
