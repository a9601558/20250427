...
// 题目更新的内部辅助函数
const updateQuestionsLogic = async (questionSetId, questions) => {
...
const getQuestionSetById = async (req, res) => {
    try {
        ...
        const questionSet = await QuestionSet_1.default.findByPk(req.params.id, {
            ...
            order: [
                [{ model: Question_1.default, as: 'questionSetQuestions' }, 'orderIndex', 'ASC'],
                [{ model: Question_1.default, as: 'questionSetQuestions' }, { model: Option_1.default, as: 'options' }, 'optionIndex', 'ASC']
            ]
        });
        if (!questionSet) {
...
const updateQuestionSet = async (req, res) => {
    try {
        ...
        if (questionSet) {
            ...
            const { title, description, category, icon, isFeatured, featuredCategory, isPaid, price, trialQuestions, questions, // 添加questions字段以防止传递给数据库
            ...
            if (hasChanges) {
                ...
                // 使用update方法直接更新数据库
                await QuestionSet_1.default.update(updateData, {
...
const deleteQuestionSet = async (req, res) => {
    ...
    try {
        ...
        const activePurchases = await Purchase_1.default.count({
            where: {
        ...
        const anyPurchases = await Purchase_1.default.count({
            where: {
                questionSetId: questionSetId
            }
        });
        ...
        const redeemCodes = await RedeemCode_1.default.count({
            where: {
                questionSetId: questionSetId
            }
        });
        ...
        const userProgress = await UserProgress_1.default.count({
            where: {
                questionSetId: questionSetId
            }
        });
...
const setFeaturedQuestionSet = async (req, res) => {
    try {
        ...
        if (questionSet) {
            ...
            // 使用静态方法直接更新
            await QuestionSet_1.default.update({
                ...
                if (Array.isArray(setData.questions) && setData.questions.length > 0) {
                    ...
                    for (const q of questionsWithId) {
                        ...
                        if (existingQuestion) {
                            ...
                            await Question_1.default.update({
                                text: q.text || '',
...
const getQuestionSetsByCategory = async (req, res) => {
    ...
    try {
        ...
        try {
            // 使用原始 SQL 查询获取分类题库
            const [results] = await database_1.default.query(`SELECT * FROM question_sets WHERE category = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`, {
                replacements: [decodedCategory, parseInt(req.query.limit) || 10, parseInt(req.query.page) || 1],
                type: sequelize_1.QueryTypes.SELECT
            });
        ...
        try {
            ...
            for (let i = 0; i < questions.length; i++) {
                ...
                if (questionData.options && Array.isArray(questionData.options)) {
                    ...
                    console.log(`为问题 ${i + 1} 创建了 ${optionsData.length} 个选项，正确答案:`, optionsData.filter((opt) => opt.isCorrect).map((opt) => opt.optionIndex).join(', '));
                }
                createdQuestions.push(newQuestion);
            }
            await transaction.commit();
